...
class TestActiveRecrodCompatibility < Minitest::Test
  ...
  def test_derive_the_same_key
    ...
    rom_derived_key = RomEncryptedAttribute::KeyDerivator.new(secret: TestData::PRIMARY_KEY, salt: TestData::DERIVATION_SALT).derive(cipher.key_len)
    assert_equal active_record_derived_key, rom_derived_key
  end

  def test_record_created_in_active_record_can_be_read_from_rom
    note = TestData::ActiveRecord::SecretNote.create!(title: "Note from AR", content: "I'm top secret")
  ...
  def test_record_create_in_rom_can_be_read_from_active_record
    note = @repo.create(title: "Note from ROM", content: "You can't read me, even if you steal the database")
